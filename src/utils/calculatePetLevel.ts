/**
 * Credits of pet level calculation to Senither/hypixel-skyblock-facade.
 *
 * Thank you!
 */
const levelOffsets = {
...
export function calculatePetLevel(tier: string, exp: number): number {
  const offset = levelOffsets[tier];
  ...
  const xpCurrent = Math.floor(exp - totalExperience);
  const xpForNext = Math.ceil(levels[level - 1]);

  return Math.floor(level + Math.max(0, Math.min(xpCurrent / xpForNext, 1)));
}
